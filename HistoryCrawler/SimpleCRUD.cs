...
namespace Dapper
{
    ...
    public static partial class SimpleCRUD
    {
        ...
        public static int? Insert<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null, bool noIntIdentifier = false, string onConflict = "")
        {
            return Insert<int?, TEntity>(connection, entityToInsert, transaction, commandTimeout, false, noIntIdentifier, onConflict: onConflict);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The ID (primary key) of the newly inserted record if it is identity using the int? type, otherwise null</returns>
        public static int? InsertOrUpdate<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null, bool noIntIdentifier = false, string onConflict = "")
        {
            return Insert<int?, TEntity>(connection, entityToInsert, transaction, commandTimeout, onConflict != "", noIntIdentifier, onConflict);
        }
        ...
        /// <param name="commandTimeout"></param>
        /// <returns>The ID (primary key) of the newly inserted record if it is identity using the int? type, otherwise null</returns>
        public static int? InsertOrIgnore<TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null)
        {
            return Insert<int?, TEntity>(connection, entityToInsert, transaction, commandTimeout, true, false);
        }
        ...
        public static TKey? Insert<TKey, TEntity>(this IDbConnection connection, TEntity entityToInsert, IDbTransaction transaction = null, int? commandTimeout = null, bool insertOrUpdate = false, bool noIntIdentifier = false, string onConflict = "")
        {
            ...
            if (insertOrUpdate)
            {
                sb.AppendFormat("insert or replace into {0}", name);
            }
            else
            {
            ...
            if (!noIntIdentifier && ((keytype == typeof(int) || keytype == typeof(long)) && Convert.ToInt64(idProps.First().GetValue(entityToInsert, null)) == 0))
            {
                sb.AppendFormat(_getIdentitySql, idProps.First().Name);
            }
            else
            {
                keyHasPredefinedValue = true;
            ...
            return r.Any() ? (TKey)r.First().id : default;
        }
